...
- Next: v0.1 once `gh auth refresh -s workflow` lands.
### 2026-09-07 — Daily dev session #71 — push/pull work on the same branch

- v0.1 still blocked: token scopes re-checked today (no `workflow`). Coverage-hunt session on the 0%-covered files from #69 (remote.go, vcs.Push/Pull).
- Found a cross-machine sync bug by probing git 2.39 behavior: both push and pull used a bare HEAD refspec, which only works when both ends name their branch the same. A fresh `git init` here defaults to `master`, GitHub creates empty remotes with HEAD at `main`. Verified empirically: second machine's `nestor pull` dies with "couldn't find remote ref HEAD", while pushes silently fork the sync onto a branch no clone-and-pull machine ever reads.
- Fix in vcs: Branch() via symbolic-ref (works on unborn HEAD, where rev-parse can't — also probed), RemoteDefaultBranch() = advertised HEAD branch, else the remote's ONLY branch (local bare remotes return an empty symref for a dangling HEAD — discovered when my own regression test caught the push forking anyway), EnsureUnbornAligned() adopts that name while HEAD is unborn; wired into both cmd flows after remote setup, BEFORE the first commit bakes the name in.
- Three deeper layers the e2e test forced out: first pull into an unborn repo needs a baseline commit (identical .gitignore on both ends blocks the merge as untracked-clobber) + --allow-unrelated-histories; git ≥2.34 refuses to guess pull.rebase on divergence (pinned --no-rebase: nestor owns both ends, merge only); merge commits died on virgin machines with "please tell me who you are" (pull subprocess now gets the same nestor@local identity fallback Commit uses).
- 4 new tests: Branch unborn/born/non-repo contract (host default overridden via GIT_CONFIG_GLOBAL), vcs-level align+push+pull round trip, empty-remote pull still fails loudly, and a full end-to-end through runPushOut/runPullOut across two isolated HOMEs (push from a master-default machine, pull onto a fresh one, branch adoption asserted, phantom-branch check, B edits and re-pushes).
...
- Next: v0.1 once `gh auth refresh -s workflow` lands.
### 2026-09-12 — Daily dev session #74 — rc source lines point at real files

- v0.1 still blocked: token scopes re-checked today (no `workflow`). Bug-hunt session, cmd/ coverage gaps (configureShell at 0%).
- Probe confirmed the bug before touching code: `SourceLines` hard-coded `source <repo>.zsh`, but the oh-my-zsh convention ships `<repo>.plugin.zsh` — cloned alias-tips locally and verified `alias-tips.zsh` does not exist while `alias-tips.plugin.zsh` does. Every `.plugin.zsh`-convention plugin got a dead source line into the managed rc block on every `up`: shell-startup error, forever, until the user hand-edits.
- Fix: new `resolveEntry(dir, repo)` resolves the entry against the clone on disk — `<repo>.zsh`, then `<repo>.plugin.zsh`, then a single `*.plugin.zsh` glob (ambiguous glob = no guess); `SourceLines` now returns `(lines, unresolved)`, and `configureShell` warns per unresolved plugin instead of writing the broken line.
- 4 new tests: shell-level SourceLines rewrite around real seeded dirs (three conventions + unresolved + dedup; old test had pinned the bug with /fake paths), resolveEntry contract incl. directory-named-like-entry trap, ambiguous-glob-unresolved; cmd-level: full `up` shell path through real `InstallPlugins` cloning a local bare repo, asserting the resolved line lands in the rc and the unresolved plugin warns without writing a source line.
...
- Next: v0.1 once `gh auth refresh -s workflow` lands.
### 2026-09-15 — Daily dev session #77 — push heals the remote's dangling HEAD

- v0.1 still blocked: token scopes re-checked today (no `workflow`). Bug-hunt session on the vcs/remote surface (the coverage gaps around #71's alignment code).
- Found by probing git on a composition #71 didn't cover: machine A commits offline on its host default (master), THEN adds the remote and pushes. The first push creates refs/heads/master on a GitHub-style bare remote whose HEAD sits at main — and git never repoints HEAD on push (`push -u` verified: no effect). The remote HEAD dangles forever: every git-native clone warns "remote HEAD refers to nonexistent ref" and checks out an empty worktree; #71's RemoteDefaultBranch one-branch fallback kept nestor's own pull/align working, so nothing ever noticed.
- Probes first: set-head --auto can't decide ("Cannot determine remote HEAD") — no client-side cure exists; server-side symbolic-ref confirmed as the fix; and the transport quirk that shaped the API: over local/file transports ls-remote --symref prints NO symref line for a dangling HEAD (exit 0, empty) — the actionable signal is "no advertised ref + ≥1 real branch".
- Fix: vcs.DanglingRemoteHEAD (detection, transport-aware) + vcs.HealRemoteHEAD ((advisory, error) — heals path-local bare remotes directly via symbolic-ref on the resolved URL; scheme/scp URLs return an advisory with the host-settings remedy); cmd push calls it after a successful push and warns on either non-empty result. Healthy, empty, and unreachable remotes stay silent.
- 3 new tests: dangling detection after the mismatched first push (+ empty-remote healthy case); heal round trip proving the cure on both consumers (git clone checks out A's work; nestor pull lands on A's branch — clone-before asserted empty); cmd-level 3 subtests (path-local healed silently, file:// URL warns with remedy, healthy remote silent).
...
- Next: v0.1 once `gh auth refresh -s workflow` lands.
### 2026-09-19 — Daily dev session #81 — config/manifest writes are crash-safe

- v0.1 still blocked: token scopes re-checked today (no `workflow`). Write-path sweep session: inventoried every file writer in the codebase (init/edit guarded first-writes; everything else truncates in place).
- Bug: every REWRITE of an existing file — writeConfig (add ×6 + import), sync's config write, snapshot's writeAtomic (manifest), restore.Write (--force) — used bare os.WriteFile. A crash or full disk mid-write leaves a truncated file: half a nestor.yml bricks every later command (config.Load refuses, sync's own invalid-config guard then blocks the only repair path), half a manifest makes the snapshot unlistable and unrestorable, restore --force destroys the only copy of the user's config.
- Fix: new internal/fsutil.WriteFileSync — MkdirAll parent (sync's virgin-machine guarantee kept), temp file in the same dir, write+fsync, chmod perm (error is real, mode 0 allowed), close, rename, best-effort dir fsync. Deferred cleanup removes the temp on any failure; dest stays byte-for-byte unchanged. All five sites delegate; writeAtomic becomes a thin wrapper at 0600 (snapshot perms tightened for free — manifests were 0644-before-umask).
- 6 new tests: fsutil round trip + default-perm write + failure injectors (parent-is-file blocks CreateTemp; dest-is-directory blocks rename — the crash analogue, asserting the child survives) + no-temp-litter on success; cmd writeConfig round trip parseable + creates missing ~/.config/nestor; restore --force durable overwrite; snapshot manifest re-parses after a second create, no litter in the base tree.
...
- Next: v0.1 once `gh auth refresh -s workflow` lands.
### 2026-09-21 — Daily dev session #83 — symlink deploy no longer removes dest before it can link

- v0.1 still blocked: token scopes re-checked today (no `workflow`). Started from the ignored-error sweep; the real find was three lines below its biggest hit.
- Bug: dotfiles' symlink path did `_ = os.Remove(dest)` BEFORE creating the link. Post-#82 the always-eats-dest window is gone (symlink failure no longer falls back into dest destruction mid-swap — verify: the only failure branch left after the remove now runs BEFORE any remove... except one, see fix), but the order still had a destroy window: a crash or kill between remove and link lost the dest outright, and a foreign file created in the window was silently linked over. The error paths also contradicted #82's durable-write doctrine by leaving destruction as step one.
- Fix: link to `dest + ".nestor-tmp-link"` beside dest and `os.Rename` over it — the old dest is destroyed only at the instant the new link replaces it. Fallback copy now runs while dest is intact and goes through fsutil.WriteFileSync (prior fallback was non-durable, mode-less Create). If the final rename fails (non-empty dir at dest), the deploy refuses with the old dest untouched and no litter.
- 5 new tests: failed swap keeps dest + user files intact with no tmp-link litter; old copy-managed regular file is replaced by the link on strategy switch; idempotent re-link over an existing link; fallbackCopy keeps dest on rename-block failure (no .nestor-write- litter); template redeploy with a since-broken render keeps the deployed link + rendered content. Old TestSymlinkFallbackCopyOnFailure updated deliberately (root can't take EACCES, so the both-fail shape is now forced by planting a non-empty dir at the tmp-link name) and now also pins dest intactness.
...
- Next: v0.1 once `gh auth refresh -s workflow` lands.
### 2026-09-24 — Daily dev session #86 — scp-style remotes are not local paths

- v0.1 still blocked: token scopes re-checked today (no `workflow`). Coverage-map session: cmd glue exempt, the 0%/low cluster sat in internal/vcs (IsUnborn, alignUnbornWithRemote, HealRemoteHEAD — the #77 unborn-HEAD machinery). Reading HealRemoteHEAD surfaced the bug two layers down.
- Bug: isLocalRemotePath classified a remote as local unless it held "://" or "@". Git's second remote syntax, scp-like host:path, carries neither when the user part is absent — probed: `git ls-remote example.com:foo/bar.git` dials SSH (connection timed out against a dead host), same for `foo:bar` and `[::1]:2222/x.git` (also missed by the @ check). Consequence: HealRemoteHEAD on a dangling user-less scp remote skipped the advisory branch, classified the URL local, and ran `git -C example.com:foo/bar.git symbolic-ref HEAD ...` — a hard error from the push-success path, violating HealRemoteHEAD's "warn, never fail the push" contract. Silent today only because detection needs ls-remote to succeed, which an unreachable host denies — but a resolvable host with an empty repo dangles and hits the branch dead-on.
- Fix: faithful port of git's own classifier (connect.c url_is_local_not_ssh): local = no scheme AND no colon before the first slash, Windows drive letters (C:/x) excepted. Drive-letter carve-out matters for the Windows build.
...
- Next: v0.1 once `gh auth refresh -s workflow` lands.
### 2026-09-26 — Daily dev session #88 — import speaks the installer's dialect

- v0.1 still blocked: token scopes re-checked today (no `workflow`). Coverage-map session again: IsUnborn 0% led into the unborn/HEAD machinery, but no bug there; the sweep landed on the importer instead.
- Bug: `nestor import brewfile` emitted package specs in a dialect nothing else spoke — "homebrew: git", "homebrew/cask: x", "homebrew/tap: x" — while the install dispatcher (packages.NewManager) only accepts brew/apt/dnf/pacman/snap. Every imported brew entry failed `nestor up` with "unsupported package manager: homebrew", ci warned on each line, and `brew list --tap` shells out a nonexistent flag. Root cause: ci carried a 20-line hand-rolled copy of the spec parser (parsePkgSpec + trimSpace + indexByte) that had drifted from the real one — the duplication WAS the bug.
- Fix: import emits canonical "brew:"/"brew/cask:" specs; taps skip (a tap is a formula repository, not an installable package); ParseSpec canonicalizes the legacy "homebrew" alias so pre-unification configs keep installing; brewMgr treats Sub=tap as always present and Install refuses rather than building "brew install --tap"; KnownManager is the single allowlist; ci deletes its parser copy and delegates to packages.ParseSpec; MergeResult dedups by parsed spec so legacy "homebrew: git" and fresh "brew: git" don't stack.
- 6 new tests: tap-not-a-package import regression, homebrew alias canonicalization, KnownManager table, tap never shells out / install refuses, InstallAll dispatches a legacy alias end-to-end through the exact brew command, ci no longer warns on alias+canonical. Two old tests flipped deliberately to the canonical contract (ManagerAndSub, brewfile import expectations) — that flip IS the fix's statement.
- Test-draft lesson struck twice in one session: heredoc `\n` inside a Go string literal becomes a REAL newline (string literal not terminated — repaired by python splice), and InstallAll's mock must fail IsInstalled but succeed Install or the batch reports already-installed and asserts on nothing.
- 449 test functions across 15/15 packages (+6, git grep HEAD 449 vs parent 443), race clean on importer+packages+ci, gofmt/vet/staticcheck clean, CGO_ENABLED=0 build clean. Pushed (ca78fbe + this journal).
- Next: v0.1 once `gh auth refresh -s workflow` lands.
